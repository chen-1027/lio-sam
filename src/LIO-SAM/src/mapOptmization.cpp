...
//下采杨点的数量
    ...
    void extractForLoopClosure()
    {
        ...
        //将surroundKeyframe中的点云放入cloudtoextract中去在找回环？
    ...
    void transformUpdate()
    {
        if (cloudInfo.imuAvailable == true)
        {
            if (std::abs(cloudInfo.imuPitchInit) < 1.4)
            {
                ...
                // slerp roll 对roll针对imu数据和优化数据进行线性差值 
    ...
    void addOdomFactor()
    {
        //加入里程计因子，如果keypose为空，加入先验因子，用keypose数组的size作为图中的index
        if (cloudKeyPoses3D->points.empty())
        {
    ...
    void addLoopFactor()
    {
        if (loopIndexQueue.empty())
            return;
        //循环检测线程保存的相关节点信息 用vector pair来保存
    ...
    void publishFrames()
    {
        ...
        // Publish surrounding key frames 点云
